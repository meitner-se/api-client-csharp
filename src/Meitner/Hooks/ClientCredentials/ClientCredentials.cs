...
namespace Meitner.Hooks.ClientCredentials
{
    ...
    public class ClientCredentialsHook : ISDKInitHook, IBeforeRequestHook, IAfterErrorHook
    {
        ...
        public IMeitnerHttpClient Client = default!;
        ...
        private Credentials? GetCredentialsGlobal(Func<object> securitySource)
        {
            ...
            if (security?.Option2?.ClientID == null || security?.Option2?.ClientSecret == null || security?.Option2?.TokenURL == null)
            {
                return null;
            }

            return new Credentials(
                security?.Option2?.ClientID!,
                security?.Option2?.ClientSecret!,
                security?.Option2?.TokenURL!,
                null
            );
        }
        ...
        private async Task<Session> DoTokenRequestAsync(string baseURL, Credentials credentials, List<string> scopes, CancellationToken? cancellationToken = null)
        {
            if( Client == null )
            {
                throw new Exception("MeitnerHttpClient not provided");
            }
